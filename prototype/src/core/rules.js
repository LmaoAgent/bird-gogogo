// 《蒜鸟的战斗》规则引擎 —— 纯逻辑,不依赖任何渲染引擎,可整体迁入 Cocos Creator。
...
export function applyGateEffect(n, effect) {
  switch (effect.type) {
 ...
 */
export function resolveSmash(n, H, k) {
  const nMin = H / k;
  return n >= nMin
 ...
 * 通关保底 1★;判负(0★)由 game.js 定,本函数只管通关后的评级。
 */
export function starRating(nPeak, targetN, thresholds) {
